// ============================================================
//  Setu AI — GenAI Legal Assistance & Analysis Engine (legal.js)
// ============================================================

/* ---- Predefined Sample Legal Documents for Quick Testing ---- */
...
function scanDocumentRisks(text) {
  if (!text || typeof text !== 'string') {
    return { score: 0, level: 'Low', flags: [], summary: 'No document text provided.' };
  }
...
function compareContracts(docA, docB) {
  ...
  // Extract lines/clauses
  const linesA = docA.split(/\n+/).map(l => l.trim()).filter(Boolean);
  const linesB = docB.split(/\n+/).map(l => l.trim()).filter(Boolean);

  const keyDiffs = [];

  // Compare key terms (Rent, Notice, Duration, Deposit, Penalty)
  const extractTerm = (text, keywordRegex) => {
    const lines = text.split(/\n+/);
    for (const l of lines) {
      if (keywordRegex.test(l)) return l;
    }
    return null;
  };
...
function generateLawyerBrief(docText, docType = 'Contract', lang = 'english') {
  ...
  // Find legal jargon present in document
  const foundJargon = [];
...
// Support Node.js CommonJS exports for test runner
